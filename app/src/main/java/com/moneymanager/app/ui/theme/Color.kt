package com.moneymanager.app.ui.theme

import androidx.compose.ui.graphics.Color

// Primary palette – deep indigo / violet
val Purple80 = Color(0xFFCDB4FF)
val PurpleGrey80 = Color(0xFFB0BEC5)
val Teal80 = Color(0xFF80DEEA)

val Purple40 = Color(0xFF5C35CC)
val PurpleGrey40 = Color(0xFF455A64)
val Teal40 = Color(0xFF00838F)

// Semantic colours
val IncomeColor  = Color(0xFF2E7D32)   // high-contrast green
val ExpenseColor = Color(0xFFC62828)   // high-contrast red

// Legacy / accent
val SalaryColor  = Color(0xFF2979FF)
val SavingsColor = Color(0xFFD500F9)

// Card / surface helpers
val CardBackground  = Color(0xFFF3F0FF)
val SurfaceVariant  = Color(0xFFEDE7F6)

// Category badge colours
val CategoryFoodColor          = Color(0xFFFF6D00)
val CategoryTransportColor     = Color(0xFF2979FF)
val CategoryShoppingColor      = Color(0xFFAA00FF)
val CategoryEntertainmentColor = Color(0xFFD81B60)
val CategoryHealthColor        = Color(0xFF00BFA5)
val CategoryUtilitiesColor     = Color(0xFFFFAB00)
val CategoryRentColor          = Color(0xFF6D4C41)
val CategorySalaryColor        = Color(0xFF00C853)
val CategoryTransferColor      = Color(0xFF1565C0)
val CategoryOtherColor         = Color(0xFF546E7A)
